...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not isWall:
                cornerStone = 0  # index of each corresponding corner in the corner list
                ...
                if cornerStone < 4:  # is in one corner
                    cornerVisited_new[cornerStone] = True  # update the corner visited list
...
def cornersHeuristic(state, problem):
    ...
    for i in range(4):  # there remains at most 4 corners unvisited, so we need to go for four timrs
        ...
        for corner in visitedlist:
            cornersDistance[cornerStone] = util.manhattanDistance(position, corners[cornerStone])
            cornerStone += 1
        closestStone = 0
        cornerStone = 0  # reset to 0
        for cornerDiatance in cornersDistance:
            if (visitedlist[closestStone]):  # in case the first corner is visited
                closestStone = cornerStone
            # check each corner: 1. not been visited 2. distance is less the current closest one => set to be the new closest one
        if (not visitedlist[cornerStone]):
            Height += cornersDistance[closestStone]
            position = corners[closestStone][:]
            visitedlist[closestStone] = True
        else:
            break
